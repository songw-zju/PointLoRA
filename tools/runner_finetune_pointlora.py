...
from utils.lora import LoRALinear, LoraConv2d
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    base_model.blocks = inject_pointlora2pt(base_model.blocks, adapter_class, gamma=8, lora_alpha=8)
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            ret, mask_loss = base_model(points)

            loss, acc = base_model.module.get_loss_acc(ret, label)

            loss += 0.004 * mask_loss
...
# inject PointLoRA layer to Point Transformer
def inject_pointlora2pt(model, adapter_class, gamma, lora_alpha):
    """
    Add adapter to point transformer
